...
public class CRC16M {
	...
	public static int update(byte[] puchMsg, int usDataLen) {
		...
		for (int i = 0; i < usDataLen; i++) {
			crc_H8 = (crc >> 8) & 0xff;
			crc = (crc << 8) & 0xffff;
			int index = crc_H8 ^ (puchMsg[i] & 0xff);
	...
	/**
	 * CRC校验
	 * @param str 待校验原始16进制数据
	 * @param len 校验位
	 * @return
	 */
	public static boolean checkCRC(byte[] str,int len) {
		int ri = update(str, len);
		int crcH = (ri >> 8) & 0xff;
		int crcL = (ri % 0x100) & 0xff;
		System.out.println(crcH+":"+(str[len]& 0xff)+"-------"+crcL+":"+(str[len+1]& 0xff));
		return (crcH == (str[len]& 0xff)) && (crcL == (str[len+1]& 0xff));
	}
}
